...
const socialLinks = [
    ...
    {
        label: "LinkedIn",
        href: "https://www.linkedin.com/in/pranay-malpani-737931278/",
        icon: (
    ...
    {
        label: "Email",
        href: "mailto:kumarmalpaniramesh@gmail.com",
        icon: (
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-4">
...
export default function Dock() {
    return (
        <div className="pointer-events-none fixed inset-x-0 bottom-0 z-30 mx-auto mb-4 flex origin-bottom h-full max-h-14">
            ...
            {/* Dock bar */}
            <div className="w-max p-2 rounded-full border border-border z-50 pointer-events-auto relative mx-auto flex min-h-full h-full items-center px-1 bg-background shadow-sm">
                {/* Nav links */}
                ...
                {/* Separator */}
                <div className="shrink-0 bg-border w-[1px] h-full mx-1" />

                {/* Social links */}
                {socialLinks.slice(1).map((link) => (
                    <DockItem key={link.label} href={link.href} label={link.label} external>
                        {link.icon}
                    </DockItem>
                ))}
...
function DockItem({
    ...
    return (
        <div className="flex aspect-square cursor-pointer items-center justify-center rounded-full" style={{ width: 40 }}>
            <a
                href={href}
                className="inline-flex items-center justify-center rounded-full size-10 hover:bg-accent hover:text-accent-foreground transition-colors"
